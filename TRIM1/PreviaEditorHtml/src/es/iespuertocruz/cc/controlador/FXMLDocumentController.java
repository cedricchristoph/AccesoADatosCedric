...
public class FXMLDocumentController implements Initializable {
    ...
    private void abrirPressed(ActionEvent event) {
        try {
            gestor.abrirArchivo();
            htmlEditor.setHtmlText(gestor.getContenidoArchivo());
    ...
    private void cerrarPressed(ActionEvent event) {
        if (gestor.isSaved()) {
            ...
            if (guardarAntes())
                try {
                    gestor.guardarArchivo(htmlEditor.getHtmlText(), true);
    ...
    private void salirPressed(ActionEvent event) {
        if (gestor.isSaved()) {
            System.exit(0);
        } else if (!(gestor.cleanHtml(htmlEditor.getHtmlText()).isEmpty())) {
            if (guardarAntes()) {
                try {
                    gestor.guardarArchivo(htmlEditor.getHtmlText(), true);
                } catch (Exception ex) {
